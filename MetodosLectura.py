def depurar_frase(archivo,depurado):
    si = '.?,¿!-_<>' 
    #guardo los caracteres que quiero quitar
    texto_limpio = open(depurado, 'a')
    with open(archivo) as texto:
        for linea in texto: 
            #recorro linea por linea del archivo sucio
            linea_limpia = linea.translate(str.maketrans(" "," ", si)) 
            #use este metodo para que cada linea sustituya lo que hay en la variable si por lo que hay entre los parentesis
...
def meter_palabras_en_diccionario(f):
    ...
    #inicializo el diccionario con el valor como lista
    for linea in x:
        frase = linea.split() 
        #guardo las palabras de una linea separas en un array
        for i in range(0,len(frase)-1): 
            #recorremos el array donde hemos guardado las palabras
            if i == 0: 
                #si es la primera la metemos en inicio
                lineas["<<INICIO>>"].append(frase[i])
            if i == len(frase)-2: 
                #si es la ultima la metemos en final
                lineas["<<FINAL>>"].append(frase[i+1])
            lineas[frase[i]].append(frase[i+1]) 
            #y aunque la hayamos metido en final o en inicio la metemos en una clave suya propia
            final = lineas["<<FINAL>>"]
    x.close()
...
def generar_palabras(diccionario,final):
    ...
    for frases in range(0,can_frases):
        ...
        for palabras in range(0,can_palabras):
            #si es la primera palabra selecciona las palabras guardadas en la clave <<INICIO>> del diccionarioç
           
            if palabras == 0:
               

                valor_random = random.randint(0, len(diccionario["<<INICIO>>"])-1) 
                #genero un numero random entre 0 y el tamaño de la clave inicio (-1 porque python acaba uno antes y si no da un error)
                palabra = (diccionario["<<INICIO>>"] [valor_random]) 
                #guardo el valor de la posicion que haya tocado de la clave inicio
                linea.append(palabra) 
                #La escribo en el archivo
            ...
            if palabras != 0 and palabras != can_palabras-1 and len(diccionario[palabra]) != 0: #el segundo condicional esta porque a veces pillaba palabras sin valor y daba un error en la libreria de random
                ...
                #guardo el valor de la posicion que haya tocado de la clave  del valor anterior
                linea.append(palabra_internas)
                palabra = palabra_internas 
                # guardo el valor de palabra interna para saber en que clave buscar del diccionario
                #palabras_generadas.write(" ")
            if palabras == can_palabras-1: 
                ...
                for y in final:
                    ...
                    if y in diccionario[palabra]:
                        ...
                        linea.append(palabra_final)
                        #palabras_generadas.write(".")
                        prueba = 0

                if prueba == 2 and len(diccionario[palabra]) != 0: 
                #Esta fuera del for para que no tenga que comprobarlo todo el rato

                    valor_random = random.randint(0, len(diccionario[palabra])-1)
                    palabra_internas = (diccionario[palabra] [valor_random])
                    linea.append(palabra_internas)
                    #palabras_generadas.write(".")
        frase_junta = " ".join(linea)
        if frase_junta not in "palabras_generadas.txt" and frase_junta not in "frases_informaticas.txt":
            palabras_generadas.write(frase_junta)
